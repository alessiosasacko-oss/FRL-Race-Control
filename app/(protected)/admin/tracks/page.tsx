          ...
          {tracks.map((track) => (
            <details key={track.id} className="master-card">
              <summary className="cursor-pointer list-none">
                <div className="flex items-center justify-between gap-4"><div><p className="eyebrow">{track.countryCode} · {track._count.races} Rennen</p><h2 className="mt-1 text-xl font-black">{track.name}</h2></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${track.active ? "bg-emerald-500/10 text-emerald-300" : "bg-slate-500/10 text-slate-400"}`}>{track.active ? "Aktiv" : "Inaktiv"}</span></div>
              </summary>
