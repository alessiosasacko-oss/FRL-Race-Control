...
function PreviewRows({ preview }: { preview: RaceFinancePreview | null }) {
  ...
  return <div className="space-y-3"><p className={`text-sm font-semibold ${preview.ready ? "text-emerald-300" : "text-amber-300"}`}>{preview.message}</p>{preview.teamTotals.map((team) => <div key={team.organizationId} className="rounded-xl border border-slate-800 bg-slate-950/45 p-3"><div className="flex items-center justify-between gap-3"><span className="font-bold text-white">{team.teamName}</span><span className={`font-black ${BigInt(team.netEuro) >= BigInt(0) ? "text-emerald-300" : "text-rose-300"}`}>{BigInt(team.netEuro) > BigInt(0) ? "+" : ""}{new Intl.NumberFormat("de-DE").format(BigInt(team.netEuro))} €</span></div><p className="mt-1 text-xs text-slate-500">Globales Opening {new Intl.NumberFormat("de-DE").format(BigInt(team.openingBalanceEuro))} € · Einnahmen {new Intl.NumberFormat("de-DE").format(BigInt(team.incomeEuro))} € · Ausgaben {new Intl.NumberFormat("de-DE").format(BigInt(team.expensesEuro))} €</p></div>)}</div>;
}
...
export default function FinanceAdminForms({ context, teams, races, drivers, rules, ruleVersion, racePreview, seasonPreview, discord }: Props) {
  ...
  return (
    <div className="grid min-w-0 gap-5 xl:grid-cols-2">
      <Link href={`/admin/finance?leagueId=${context.leagueId}&seasonId=${context.seasonId}${selectedRaceId ? `&raceId=${selectedRaceId}` : ""}&loadPreviews=1`} className="wizard-secondary-button min-h-11 w-full justify-center xl:col-span-2">Abrechnungsvorschauen laden</Link>
