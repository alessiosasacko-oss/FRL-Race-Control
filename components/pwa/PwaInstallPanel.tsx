    ...
    <section className="master-card" aria-labelledby="frl-app-title">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 id="frl-app-title" className="text-xl font-semibold text-white">FRL Race Control App</h2>
      ...
      {!prompt && !showIosGuide && !installed ? (
        <p className="mt-5 rounded-2xl border border-slate-700 bg-slate-950/40 p-4 text-sm leading-6 text-slate-300">
          Dein Browser zeigt den Installationsdialog gerade nicht an. Öffne das Browser-Menü und wähle
          {" "}<strong className="text-white">App installieren</strong> oder <strong className="text-white">Diese Website als App installieren</strong>.
        </p>
      ) : null}
    </section>
  );
