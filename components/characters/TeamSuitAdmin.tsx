    ...
    <div className="space-y-5">
      {organizations.length === 0 ? <div className="master-card text-sm text-slate-300">Noch keine Teamorganisation vorhanden. Lege zuerst ein Team an.</div> : <>
        <section className="master-card space-y-4">
          ...
          <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-300">Teamorganisation</span><select value={organizationId} onChange={(event) => selectOrganization(Number(event.target.value))} className="form-input min-h-11 w-full">{organizations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-300">Name</span><input value={name} onChange={(event) => setName(event.target.value)} className="form-input min-h-11 w-full" maxLength={120} /></label>
          ...
          <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-300">Muster</span><select value={configuration.pattern} onChange={(event) => setConfiguration((current) => ({ ...current, pattern: event.target.value as TeamSuitConfiguration["pattern"] }))} className="form-input min-h-11 w-full">{suitPatterns.map((pattern) => <option key={pattern} value={pattern}>{pattern.replaceAll("_", " ")}</option>)}</select></label>
          <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-300">Ärmelgestaltung</span><select value={configuration.sleeveStyle} onChange={(event) => setConfiguration((current) => ({ ...current, sleeveStyle: event.target.value as TeamSuitConfiguration["sleeveStyle"] }))} className="form-input min-h-11 w-full"><option value="SOLID">Einfarbig</option><option value="CONTRAST">Kontrast</option><option value="STRIPED">Gestreift</option></select></label>
