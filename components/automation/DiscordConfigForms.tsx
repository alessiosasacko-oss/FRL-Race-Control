...
import { DiscordChannelPurpose, Role, roleLabels } from "@/domain";
import {
...
export default function DiscordConfigForms({ guilds, leagues }: Props) {
  const [guildState, guildAction, guildPending] = useActionState(saveDiscordGuildAction, initialAutomationActionState);
  const [channelState, channelAction, channelPending] = useActionState(saveDiscordChannelAction, initialAutomationActionState);
  const [roleState, roleAction, rolePending] = useActionState(saveDiscordRoleAction, initialAutomationActionState);
  const defaultGuild = guilds[0];

  return (
    <section className="grid min-w-0 gap-6 xl:grid-cols-2">
      <form action={guildAction} className="master-card">
        <div className="flex items-start gap-3"><span className="rounded-xl bg-blue-600/15 p-3 text-blue-400"><Server size={21} /></span><div><h2 className="text-lg font-semibold text-white">Discord-Server</h2><p className="mt-1 text-sm text-slate-400">Bot mit dem FRL-Guild verbinden.</p></div></div>
        <div className="mt-5 space-y-4">
          <label className="master-label">Guild-ID<input name="guildId" required defaultValue={defaultGuild?.guildId} className="form-control mt-2" /></label>
          <label className="master-label">Anzeigename<input name="guildName" required defaultValue={defaultGuild?.guildName} className="form-control mt-2" /></label>
          <label className="flex min-h-11 items-center gap-3 text-sm text-slate-300"><input type="checkbox" name="enabled" defaultChecked={defaultGuild?.enabled ?? true} className="size-5 accent-blue-600" />Integration aktiv</label>
        </div>
        <div className="mt-5 space-y-3"><FormMessage state={guildState} /><button disabled={guildPending} className="wizard-primary-button min-h-11 w-full"><Save size={17} />Server speichern</button></div>
      </form>
      ...
      <form action={roleAction} className="master-card">
        ...
        {defaultGuild ? <div className="mt-5 space-y-4">
          ...
          <FormMessage state={roleState} /><button disabled={rolePending} className="wizard-primary-button min-h-11 w-full"><Save size={17} />Rolle speichern</button>
        </div> : <p className="mt-5 text-sm text-amber-300">Zuerst einen Discord-Server speichern.</p>}
      </form>
      ...
      <details className="master-card min-w-0 xl:col-span-2">
        ...
        {defaultGuild ? <form action={channelAction} className="mt-5 grid min-w-0 gap-4 border-t border-slate-800 pt-5 md:grid-cols-2 xl:grid-cols-3">
          ...
          <label className="flex min-h-11 items-center gap-3 text-sm text-slate-300"><input type="checkbox" name="enabled" defaultChecked className="size-5 accent-blue-600" />Zuordnung aktiv</label>
          <div className="space-y-2"><FormMessage state={channelState} /><button disabled={channelPending} className="wizard-primary-button min-h-11 w-full"><Save size={17} />Sonderzuordnung speichern</button></div>
        </form> : <p className="mt-5 text-sm text-amber-300">Zuerst einen Discord-Server speichern.</p>}
      </details>
    </section>
  );
}
