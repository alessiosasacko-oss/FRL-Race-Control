...
export async function getDashboardData(userId: number): Promise<DashboardData> {
  ...
  const [championship, lastResult, latestResult, seasonProgress, notifications, unreadNotificationCount] = await Promise.all([
  ...
  const [driverStanding, teamStanding, driverLeader, teamLeader] = await Promise.all([
    ...
    seasonId ? optionalDashboardData("driver leader", () => prisma.driverStanding.findFirst({ where: { championship: { seasonId, leagueId: leagueId ?? undefined } }, orderBy: { position: "asc" }, select: { points: true } }), null) : null,
    seasonId ? optionalDashboardData("team leader", () => prisma.teamStanding.findFirst({ where: { championship: { seasonId, leagueId: leagueId ?? undefined } }, orderBy: { position: "asc" }, select: { points: true } }), null) : null,
  ]);
  ...
  const data: DashboardData = {
    ...
    championship: { driver: driverStanding ? { position: driverStanding.position, points: driverStanding.points, gapToLeader: Math.max(0, (driverLeader?.points ?? driverStanding.points) - driverStanding.points), lastRacePoints: (lastResult?.racePoints ?? 0) + (lastResult?.bonusPoints ?? 0), wins: driverStanding.wins, podiums: driverStanding.podiums } : null, team: teamStanding ? { position: teamStanding.position, points: teamStanding.points, gapToLeader: Math.max(0, (teamLeader?.points ?? teamStanding.points) - teamStanding.points) } : null, topDrivers: championship?.driverStandings.map((standing) => ({ position: standing.position, name: standing.driver.name, flag: standing.driver.flag, points: standing.points })) ?? [], topTeams: championship?.teamStandings.map((standing) => ({ position: standing.position, name: standing.team.organization?.name ?? standing.team.name, color: standing.team.organization?.color ?? standing.team.color, logoUrl: standing.team.organization?.logoUrl ?? standing.team.logoUrl, points: standing.points })) ?? [] },
