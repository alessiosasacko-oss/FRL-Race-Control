...
test("11. only allowed widget sizes are offered", () => assert.deepEqual(dashboardWidgetRegistry["latest-result"].allowedSizes, ["small", "medium"]));
...
test("17. invalid sizes are rejected", () => { const layout = createDefaultDashboardLayout(); const index = layout.desktop.findIndex((item) => item.id === "latest-result"); layout.desktop[index] = { ...layout.desktop[index], size: "full" }; assert.equal(dashboardLayoutSchema.safeParse(layout).success, false); });
...
test("19. widgets requiring a driver stay unavailable without one", () => { const ids = availableDashboardWidgetIds([Role.Driver], false); assert.equal(ids.includes("championship-position"), false); });
